...
describe('glm-5.2 AI 样本 —— 真实风格 DSL 文档被 strict parser 零诊断接受', () => {
  it('树形布局(根 + right-of 子节点 + 关系箭头)', () => {
    // 注:strict 模式不接受 `#` 注释行(当 prose 报错),故 AI 样本只含纯指令行。
    const dsl = [
  ...
  it('grammar 版本号 v5(锁定 AI 样本所依据的语法版)', () => {
  ...
  describe('strict 与 graceful 对注释/散文的处理差异(锁定)', () => {
    ...
    it('strict:行首 `#` 注释被当 prose 报错(与 GRAMMAR_REFERENCE 措辞有张力,锁定现状)', () => {
      // 注意:DSL_GRAMMAR_REFERENCE 告诉 AI "Lines starting with # are comments and ignored",
      // 但 parseDslStrictWithDiagnostics(strict 模式,文档注明用于 AI 输出)把 `#` 行当
      // "unexpected prose or markdown" 报错。即 AI 若照 grammar 说明发注释,strict 会拒。
      // 这是文档措辞与 strict 行为之间的一处张力 —— 此处锁定 strict 的实际行为。
