...
- Phase 0 ✅ 脚手架 · Phase 1 ✅ 设计系统 · Phase 2 ✅ 数据层 · Phase 3 ✅ Inbox 业务 · Phase 4 ✅ Canvas 基础 · Phase 5 ✅ Canvas 完整 · Phase 6 ✅ 捕获入口 · Phase 7 ✅ Archive · P6.5a ✅ 草稿自动保存 · P6.5b ✅ Inbox 多媒介编辑 · P6.5c ✅ Inbox→Canvas Send · P6.5d ✅ 画布视图持久化 · P6.5e ✅ 统一手动 capture · P6.5f ✅ 图片上传 · P6.5g ✅ 菜单栏 + CaptureSinkRegistry · P6.5h ✅ 快捷键自定义 · Phase 8 ✅ Rust 就绪(2026-06-20:本就已装 cargo 1.96,根因是 PATH,cargo check 通过;待 tauri build/签名) · **Phase 9 ✅ JSON 导出 + 用户文档** · **Review ✅ bugfix #1+#3(import 原子性 + sink 竞态,v0.9.2)** · **Phase trash ✅ soft-delete 回收/恢复视图(/trash 路由 + domain restore/hardDelete + AppMenu Trash 入口,v0.10.0-trash)** · **Phase canvas-refactor ✅ useEffect 驱动 canvas-editor(关闭 review #4 #5,v0.11.0-canvas-refactor)** · **Phase archive-detail ✅ archive tile 接 detail Modal + 共享 CardDetailModal(关闭 review §🟠 UX #4,v0.12.0-archive-detail)**
- **30 轮路线图核心 spec §8 全部完成 + review 全部 5 项关闭 + UX 洞 #4 关闭**。Phase 8 Tauri:Rust 就绪(cargo 1.96),待 tauri build/签名。产品已是**完整可用的 web 应用**:捕获 / inbox(多媒介编辑)/ canvas(视图持久化 + useEffect 驱动)/ archive(网格+时间轴+多选+详情 Modal)/ trash(软删恢复)/ settings(快捷键自定义+导出+导入)/ 用户文档。
- **✅ review findings 全部关闭 + UX 洞 #4 关闭**: #1 #2 #3 #4 #5(commit 649ba9f / f8b71dc / aa860e7)+ UX #4(archive tile 接 Modal)。见 `docs/memory/decisions/2026-06-19-review-findings.md`(原始)+ `2026-06-20-review-bugfixes.md`(#1+#3)+ `2026-06-20-trash.md`(#2)+ `2026-06-20-canvas-refactor.md`(#4+#5)+ `2026-06-20-archive-detail.md`(UX #4)。
- **执行模式**：主模型（Claude）按 plan 手动执行 + 自审;Ralph 自动循环已停用（见下方"Ralph 状态"）
- **下一个**：等用户诉求(新功能 / Phase 8)。候选:暗色模式 / 多画布 UI / 标签搜索 / OPFS(Phase 2.5)/ 剩余 UX 洞(批量 soft-delete 二次确认 / send-to-canvas 反向动作)/ Phase 8 Tauri build(Rust 就绪)+ 签名公证(需 Apple 证书)
