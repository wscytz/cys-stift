...
- **30 轮路线图核心 spec §8 全部完成 + review 全部 5 项 + UX 洞 #2 #3 #4 全部关闭**。Phase 8 Tauri:Rust 就绪(cargo 1.96),待 tauri build/签名。产品已是**完整可用的 web 应用**:捕获 / inbox(多媒介编辑)/ canvas(视图持久化 + useEffect 驱动 + dblclick 走 registry)/ archive(网格+时间轴+多选+详情 Modal + 批量软删二次确认)/ trash(软删恢复)/ settings(快捷键自定义+导出+导入)/ 用户文档。
- **✅ review findings 全部关闭 + UX 洞 #2 #3 #4 关闭**: #1 #2 #3 #4 #5(commit 649ba9f / f8b71dc / aa860e7)+ UX #2 #3 #4(commit 9d7aa24 / 19b9b2e / 15fd29d / 9a8bf86)。**所有 review 5 项 + UX 洞 4 项全部关闭,产品已无遗留 review 项**。
