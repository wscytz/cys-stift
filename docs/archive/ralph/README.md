...
/ralph-loop "$(cat docs/plans/<phase>.md)" \
...
- **设计文档**：`docs/specs/2026-06-19-cys-stift-design.md`（五轮审查定稿）
- **路线图**：10 个 phase，spec §8
- **当前进度**：`docs/changelog.md`
- **架构决策**：`docs/adr/`
- **跨模型记忆**：`docs/decisions/INDEX.md` 索引 + `decisions/` 详情
...
读取：
- 本文档（第 5、6 节）
- `docs/plans/<current-phase>.md`（具体 phase 计划）
- `docs/specs/2026-06-19-cys-stift-design.md`（对应章节）
...
- 追加 `docs/decisions/2026-06-19-phase-N.md`
- 更新 `docs/decisions/INDEX.md` 索引
...
1. 每完成一个里程碑（或上下文到 70–80%），把当前进度写入 `docs/archive/ralph/session-handoff.md`（用 `session-handoff.template.md` 模板）
2. `git add docs/archive/ralph/session-handoff.md && git commit -m "chore: ralph handoff at <phase>"`
3. 执行 `/clear`（**不是 compact**——彻底重置，保真度更高）
4. 下一轮第一件事：读 `docs/archive/ralph/session-handoff.md` + 根 `CLAUDE.md` + 当前 phase plan，继续迭代
...
- [ ] **没碰 `docs/specs/`**（spec 是定稿，要改走 5 轮审查）
...
如果 5 轮迭代还没进展：
1. 读 `docs/changelog.md` 全文
2. 跑 `pnpm test` / `pnpm build` 看哪条挂了
3. `git log` 看上一 phase 怎么做的
4. 写 `docs/decisions/YYYY-MM-DD-<phase>-stuck.md` 说明阻塞
5. **不要碰 `<promise>`**
