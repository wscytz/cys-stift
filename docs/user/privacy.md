...
样本只保存 `question/context/aiOutput`、结果和 DSL 版号等脱敏字段,不保存原始 `Card[]`、settings、`deviceId` 或 API key。完整 JSON 备份可以包含卡片、媒体、草稿、设置、画布几何、对话和样本,所以分享前请把它当作包含你内容的文件处理。**画布 DSL 文本(转义 / DSL 模态编辑器 / 复制导出)**自 v5 起携带卡片 `@title`/`@content`(此前仅几何),与 JSON 导出同列——分享 DSL 文本同样当作包含卡片内容处理(但 DSL 永远不含 `apiKey`、媒体二进制或手绘点序列)。
