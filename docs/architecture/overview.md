...
- `apps/desktop` — Tauri v2 桌面壳(包 web 静态产物 + 全局快捷键 + 未来 `fs` 落盘)
- `packages/ui` — 包豪斯设计系统 + React 组件
- `packages/db` — Drizzle ORM + better-sqlite3 schema(Node 路径;web 暂走 localStorage adapter,为 Tauri 预留)
- `packages/domain` — 纯 TS 业务规则(零依赖,Repository 接口注入)
- `packages/config` — 共享 tsconfig(占位)
...
1. **本地优先**——数据在客户端,Web/Desktop 共用一套 domain 代码,离线可用。
2. **Card 为唯一真相源**——画布上的 tldraw card shape 只存几何 + cardId 引用,内容渲染查 CardService;inbox/archive 编辑实时反映到画布。
