...
- **画布自研 · 路线 A(Phase 0 完成,T0.1–T0.4 + 真实 tldraw 冒烟 e2e 已提交)**:渐进 tldraw → 自研 Canvas 2D 渲染器;特色 = 几何元素双向文本 DSL + 手绘向量存储。计划 `docs/plans/2026-06-22-canvas-self-build-route-a.md` + Phase 0 逐步计划 `docs/plans/2026-06-22-canvas-self-build-phase0.md`;调研 `docs/decisions/2026-06-22-canvas-research-drawio-archdiag-affine.md`;冒烟 e2e `scripts/phase0-smoke.cjs`。**Phase 0 产出**:`CanvasHost` 引擎无关接口 + `TldrawAdapter`;核心业务逻辑(canvas-binding/apply-layout)+ AI 路径(canvas-snapshot)已改依赖 host,grep `@tldraw` 只剩 host/tldraw-adapter.ts;双向 DSL(serializeCanvas ↔ parseDsl)round-trip 上线;真实 tldraw 冒烟 5/5(挂载/同步/拖拽回写/橡皮软删)。**剩余 tldraw 耦合 = Phase 1/2 引擎集成(非业务前置)**:card-shape-util(ShapeUtil)、`<Tldraw>` 挂载、导出层(getSvgString)、`.cystift`(getSnapshot)、工具系统、handle/binding、命中测试/选择/坐标、快照持久化——这些是渲染器自己的事,Phase 1(建渲染器)/ Phase 2(拆 tldraw)本就重建,不当前置硬迁。**下一步:Phase 1 基础骨架 plan**(自研 Canvas 2D 渲染器,feature-flag 并存;`docs/plans/2026-06-22-canvas-self-build-phase1-foundation.md`)。移除 tldraw(Phase 2)走 ADR + spec §3.4/§6.x 五轮审查。
