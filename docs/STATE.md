...
> 版本表由 `scripts/gen-state.mjs` 从 `git tag` 生成,不会漂移。最后更新:v0.55.0。
...
1. ~~UI 一贯性扫描~~ ✅ 已落地(2026-07-03 批 A `735fbc1` + 批 B `130a996`):跨页 tile/按钮/间距/色/焦点环/配方一致性 + tile 两份 CSS 结构债清除 + LOW 快赢(select dark 去重/confirm focus bug/canvas-engine fallback 色/scrim token 等 10 条)。详见 changelog `ui-consistency-polish`。
...
- **v0.41 批 2+3(2026-06-30,P1 交互 + P1 UI)**:批 2 交互:① **resize snap**(snap 模式拖角尺寸落 8 倍数,drag 吸附 resize 不吸附的手感不一致);② **lazy pushUndo**(select/resize 纯点击不拖 / eraser 点空白不再推空 undo 步,undo 栈不再被频繁选择污染);③ **RelationPanel auto-apply toast**(选中无标签可推断箭头自动应用关系类型后弹一次 info toast,键控不刷屏)。#5 eraser card redo defer(undo 已工作,redo 是罕见边缘+预存限制)。批 3 UI:error.tsx/markdown 嵌入提示 i18n(error.boundary.* + md.embed.* 键)+ agent-confirm-card 缩略图 hex 改 readToken(暗色可见)+ tag-cloud 计数色 rgba 改 token(暗色可读)+ toolbar 标签字号走 var(--font-size-xs)。canvas-engine 463 / web 926 全绿;lint exit 0;build exit 0。批 4(P2 打磨)待续。详见 changelog。
- **v0.41 批 1(2026-06-30,P0 + v0.40 遗漏)**:工具栏工具逻辑 + 画布交互系统审计(3 subagent,24 条 backlog `docs/specs/2026-06-30-v041-polish-backlog.md`)。批 1 = 4 条 v0.40 同类根因漏网,全集中在 `self-built-adapter.ts`:① **visibilitychange 清残留**(Tab 切走/失焦浏览器不发 pointerup/cancel,dragGroup/currentStroke/connecting 残留 → 回来幽灵移动;挂 window visibilitychange,hidden 时 clearInteractionState);② **方向键 undo 粒度**(按住 OS 自动重复 fire keydown,每次推快照致栈爆炸;首次 repeat=false pushUndo+coalescing,repeat 跳过,keyup 关);③ **freedraw pointercancel discard**(与 v0.40 connect cancel 完全同构,onCancel 加 freedraw 分支不 commit 半截笔画);④ **connect hitTest 用缓存**(v0.40 双层缓存修了 renderNow,onMove 的 hitTestCardWithTolerance 仍每帧 getElements() 排序,改 getSortedElements())。批 2-4(交互/UI/打磨)待续。canvas-engine 455 / web 926 全绿;lint exit 0;build exit 0。详见 changelog。
- **v0.40 交互打磨批(2026-06-30,v0.40.0)**:v0.40.0 手测反馈三块打磨,spec `docs/specs/2026-06-30-v040-polish-design.md` + plan `docs/plans/2026-06-30-v040-polish.md`,subagent 逐任务 TDD。**B 选择态清理**:`switchCanvas` 显式 `setSelectedIds([])`(不只靠 key 重建副作用)+ `usePathname` effect 路由离开 /canvas 清(用户实测残留场景)。**A 箭头拖拽手感**:① pointercancel 对 connect 走丢弃路径(旧逻辑复用 onUp 在坏坐标 hitTest,系统中断致预览突消/建错箭头);② 松手容差 `hitTestCardWithTolerance`(card bbox 外扩 6px,偏几像素仍连上)+ move 中目标高亮(`connecting.toId` 跟踪,蓝边描边反馈);③ **双层渲染缓存**(层排序 + 视口剔除,版本计数器失效,修大画布拖箭头卡顿 —— 初版经 getElements() 查缓存形同虚设,改直接读 this.elements Map)。**C 图谱状态维持**:新 `graph-view-store`(照搬 canvas-view-store 范式,视口+节点坐标 localStorage 持久化)+ `createGraphSimulation` 参数化 `initialPositions`(去无条件 Math.random)+ GraphCanvas mount 恢复视口/坐标 + view/坐标 throttle 200ms 回写 + prunePositions 清已删节点。canvas-engine 448 / web 926 全绿;build exit 0;tsc 零新增(23 预存在 fixture 基线)。详见 changelog。
