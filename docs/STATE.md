...
> 版本表由 `scripts/gen-state.mjs` 从 `git tag` 生成,不会漂移。最后更新:2026-07-23。2026-07-21 回退桌面端 `security.csp`(→ `null`)修复 Tauri 构建渲染静默失效;`v1.0.0` 发布资产本身为 `csp: null`,未受影响(详见 changelog)。当前源码版本为 **1.1.0**（根 `package.json` 单一来源）；`v1.0.0` 是 GitHub stable release，tag workflow 重新运行 Linux gate、生成 Windows x64 NSIS、生成并验证 macOS Apple Silicon ad-hoc DMG，并发布平铺可校验的 `SHA256SUMS.txt`。该正式发布当时冻结核心工作流、数据格式与 cys-dsl v4；**post-release `feat/cys-dsl-extract` 当前源码已升级到 cys-dsl v7**(独立包 + card `@title/@content` + 纯内容编辑/清空 + freedraw 出 DSL + v7 `@group/@href/@compute` + app 全路径适配),已合并 main、版本 bump 1.1.0(尚未 tag);v1.0.0 发布事实不变。macOS 无 Apple Developer ID/Team ID 和 notarization，Windows 无 Authenticode 签名；Android 不在 1.0.0 支持范围。VoiceOver、真实系统 200% 缩放、代表性设备安装升级、真实 provider quota/refusal 和外部用户研究仍是发布后加固项。
...
- **cys-dsl v7(post-release 分支)**:DSL 已抽到 `packages/cys-dsl`(纯逻辑包);v5 加 card `@title/@content`,v6 将 freedraw 移出 DSL(程序自管 R2 + 渲染,DSL 5 kind),**v7 加三条 directive**:`@group("名")`(语义分组,落 `element.meta.group`,组样式/折叠是视图层不进 DSL)、`@href(#a;#b)`(卡片显式语义引用,落 `meta.href`,不画线、区别于正文 `[[wikilink]]`→自动箭头)、`@compute("公式")`(仅 text,**受限递归下降求值器禁裸 eval**,只引用几何 `#id.x|y|w|h` 不碰卡片内容,apply 时求值写 text,每次 apply 重算非 live)。parse → sanitize → relational solve → plan → confirm/apply 全链适配内容:create/update 写回 CardService、回滚/一次性 undo 覆盖 title/body、空串清空、无 `@pos` 的纯内容/属性编辑沿用现有几何;strict 放行 `#` 注释但拒散文。手动 DSL 编辑器显示全量内容 token;copy-DSL/模板不注入 content(消费者策略,纯几何);`/ask` prompt 已使用 v7 grammar 并可按任务生成/修改内容;freedraw 不进 DSL 但 AI snapshot 仍可看本地 shape 描述符。**v7 做圆(2026-07-23,A 计划)**:`@compute` 前向引用修(`buildApplyPlan` post-pass 用完整 shadow 重算 + 失败诊断,加 `evalComputeDetail`);`@group` 组色带(画布 card 左边条,同组共享组色 hash→4 原色 token,canvas+SVG);`@href` 端点标记(graph 节点 ● + hover 临时无头灰细连线,不画常驻线,与 arrow 零重叠,加 `aggregateHrefs`)。
