...
- **画布自研 · 路线 A(Phase 0 + Phase 1 全完成 + Phase 2 子项目1 完成)**:渐进 tldraw → 自研 Canvas 2D 渲染器;特色 = 几何元素双向文本 DSL + 手绘向量 + 关系箭头(渲染+connect)+ 文本编辑(IME)+ 选择/删除/resize/多选组移动/键盘微移/undo-redo。计划 `docs/plans/2026-06-22-canvas-self-build-route-a.md` + Phase 2 spec `docs/superpowers/specs/2026-06-23-phase2-canvas-replace-tldraw.md` + 逐步计划(phase0 / phase1-foundation / `-freedraw` / `-arrow` / `-text` / `-selection` / `-resize` / `-multiselect` / `-connect` / `-keyboard` / phase2-sub1);调研 `docs/decisions/2026-06-22-canvas-research-drawio-archdiag-affine.md`;冒烟 `scripts/phase0-smoke.cjs` / `phase1-{,freedraw-,arrow-,text-,selection-,resize-,multiselect-,connect-,keyboard-}smoke.cjs` / `phase2-main-smoke.cjs`。**Phase 0**:CanvasHost + TldrawAdapter,核心业务逻辑 + AI 路径上 host。**Phase 1**:SelfBuiltAdapter(Canvas 2D,零 tldraw)功能完备(渲染+拖拽+pan/zoom+手绘+文本IME+选择+Delete+resize+多选组移动+框选+connect+键盘微移+全选+undo/redo)。**Phase 2 子项目1(基础接通 keystone,已完成)**:主路由 `/canvas` 切 `<SelfCanvas>`(SelfBuiltAdapter 主路由版),移除 page tldraw 依赖;接通 CardService/多画布/视图持久化/双击开卡/发回/归档/软删/AI 布局;主路由冒烟 3/3(拖拽回写)。tldraw 代码文件暂留(子项目5 删)。300 web 测试 + 11 套真实 Chrome 冒烟。**Phase 2 剩余子项目**:子2 卡片完整渲染+toolbar / 子3 导出层迁 CanvasElement / 子4 关系 panel+auto-relate / 子5 移除 tldraw(ADR + spec §3.4/§6.x 五轮审查 + 删依赖)。**子项目1 后主路由暂缺**:toolbar 工具入口/导出/关系 panel/卡片完整渲染(子2-4 接回)。已知 token gap:`--color-green`/`--color-canvas` 未定义(readToken 回退兜底,无害);undo 粒度=每次变更 1 步(无 coalescing,50 步上限兜底);视图持久化 500ms 轮询(临时,子2 改事件)。
