...
- **画布自研 · 路线 A(Phase 0 + Phase 1 全完成 + Phase 2 子项目1-3 完成)**:渐进 tldraw → 自研 Canvas 2D 渲染器;特色 = 几何元素双向文本 DSL + 手绘向量 + 关系箭头(渲染+connect)+ 文本编辑(IME)+ 选择/删除/resize/多选组移动/键盘微移/undo-redo + 导出(SVG/PNG/.cystift 全走 CanvasElement)。计划 `docs/plans/2026-06-22-canvas-self-build-route-a.md` + Phase 2 spec `docs/superpowers/specs/2026-06-23-phase2-canvas-replace-tldraw.md` + 逐步计划(phase0 / phase1-foundation / `-freedraw` / `-arrow` / `-text` / `-selection` / `-resize` / `-multiselect` / `-connect` / `-keyboard` / phase2-sub1 / -sub2 / -sub3);调研 `docs/decisions/2026-06-22-canvas-research-drawio-archdiag-affine.md`;冒烟 `scripts/phase0-smoke.cjs` / `phase1-{,freedraw-,arrow-,text-,selection-,resize-,multiselect-,connect-,keyboard-}smoke.cjs` / `phase2-{main,sub2,sub3}-smoke.cjs`。**Phase 0**:CanvasHost + TldrawAdapter,核心业务逻辑 + AI 路径上 host。**Phase 1**:SelfBuiltAdapter(Canvas 2D,零 tldraw)功能完备。**Phase 2 子1**:主路由切 SelfCanvas。**子2**:card 完整渲染 + toolbar + onViewChange。**子3(导出层,原生层优势落点)**:`elementsToSvg`(CanvasElement→SVG,零 tldraw,复用字体嵌入/图片内联)+ 自研光栅化(SVG→img→canvas→blob)+ `.cystift` 几何透明 CanvasElement[](替代 opaque tldraw snapshot)+ export-dialog 改 host + 主路由接回 Export 入口。导出层 grep `@tldraw` 归零。310 web 测试 + 13 套真实 Chrome 冒烟。**Phase 2 剩余**:子4 关系 panel+auto-relate / 子5 移除 tldraw(ADR + spec §3.4/§6.x 五轮审查 + 删依赖)。**子3 已知限制**:`restoreCystiftPayload` 的 host 参数可选(file-drop-handler 无 host,降级只恢复 cards;几何元素 reload 持久化留后续)。**子3 后主路由暂缺**:关系 panel(text 编辑接主路由也仍缺)。已知 token gap:`--color-green`/`--color-canvas` 未定义;undo 粒度=每次变更 1 步。
