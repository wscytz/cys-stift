...
- **已知待修(下次任务 #210/#211)**:① 吸附失效——snap 从未实装在拖动路径(gridMode 只驱动按钮,drag 不读);② 转义后卡片漂移——`apply-layout.ts:112-113` 的 `Math.max(0,…)` 钳掉负坐标(serialize/parse 忠实往返)→ 拖到原点左上的卡 round-trip 跳到 (0,0),且 writeback 把错坐标写进 DB(刷新后永久错位)。根因 + 修法已落 TaskList + 记忆 `snap-and-dsl-drift-rootcauses.md`。
