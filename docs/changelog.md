...
- **无障碍(键盘可达)**:图谱视图与画布建卡可用键盘完成;焦点管理、Esc 一致取消统一收口。画布工具浮层不再遮挡操作目标。
- **画布手感**:建卡可撤销;text 工具能在 frame 内放字(此前 frame 占满命中导致放不下);橡皮"模式不匹配"只在真命中元素时提示,空点击不再误报;滚轮/触摸板缩放跟随实际滚动量(此前固定倍率,同手势缩放幅度受拆帧影响);选中关系箭头时双击空白可继续建卡。
- **找回(⌘K)**:键盘全程导航 + 命中高亮;链接卡片标题纳入搜索;筛选条件与结果不再误导。
- **导入导出**:导入前恢复点可控、导出有明确反馈、拖回画布有引导、错误文案可读。**导入恢复边界**:当存储空间不足以写完整快照时,导入照常进行并提示"未创建恢复点"(新状态更小仍能容纳);事务回滚由独立快照保证,不依赖恢复点。修复编辑 `@links` 字段时的有损重建(URL 未变的链接保留其标题/预览/抓取时间)。
- **存储感知**:用量按真实 localStorage 预算计算;清理路径明确;大文件有提示;数据位置可在设置中查看。
- **关系体验**:建连即定类型;双链方向一致;图谱聚焦;删除有提示。
- **深度编辑**:工作台切卡时富字段(`@code`/`@quote`/`@links`)不丢;三态(草稿/已存/失败)保持位置;IME 组合态不误触发;状态语义清晰。
- **跨页一致性 + 交互打磨**:详情动作集、标签手势、删除文案、保存反馈、置顶跨页一致;确认门、快捷键冲突、Esc 叠关、confirm 统一。
- **测试与门控**:补 `@code`/`@quote` 往返与 AI 字段白名单守卫(allowlist 不能静默缩水);新增 puppeteer e2e `e2e:canvas`(text-in-frame + 橡皮误报对照);导入恢复边界单测覆盖;CI docs-sync 在缺 token 时优雅跳过。
- 验证基线:全包 lint 0 / 全包 test 全绿 / web build exit 0(测试总数以 CI 完整输出为准,不硬编码)。
- **桌面签名状态不变**:Windows 无 Authenticode、macOS ad-hoc 未公证,安装前核对 SHA256。
...
打包分发版。围绕产品方向(转义 DSL 差异化 + 本地优先画布)推进 + 健壮性 + 文档对齐。
...
- **竞品对标报告(内部 docs/research/)**:多维度品类对标 + cy's Stift 特性验证(内部决策用,结论存私有仓)。
...
product-idea #4 Markdown 小尾。MarkdownBody 加数学公式 + 脚注;单一渲染器(工作台编辑器预览联动)。TOC 不做(卡片 YAGNI)。spec:内部 spec;plan:内部 plan。
...
「展开工作台」后的深度编辑加**二档**:一键编辑器撑满,画布缩成可拖拽 / 可收起(剩角)的 minimap 预览。给长文 / 复杂修改一个沉浸式空间,画布仍作被动参考常驻一角。spec:内部 spec;plan:内部 plan。6 task subagent-driven(T1-T5 shipped `4527ac7..58ddaec`,T6 本轮 wrap-up)。
...
STATE 缺口 ⑮ 的 5 项 wikilink 延后,一次性做完(C 范围全 5 项)。spec:内部 spec;plan:内部 plan。6 task subagent-driven(T1-T5 `c62278d..946314f` shipped,T6 本轮 wrap-up)。
...
给 cy 一个**本地开发存档系统** —— 在 release / 风险 op / 手动 checkpoint 时刻自动落全量状态快照(OPFS,带版号),供查档 / 查错误 / release 自修自查。spec:内部 spec;plan:内部 plan。7 task subagent-driven(T1-T6 已落地 `bf37a72..a7002b2`,T7 本轮)。
...
「大卡扶正」简化版第一阶段：富 Markdown + 工作台 dock 编辑器。spec 见私有仓 内部 spec，计划见 内部 plan。简化定调：**不引入新卡类型、不做 AI 角色卡**——"大"由内容撑。
...
用户需求:箭头要能弯曲(单控制点光滑曲线)和折线(单/双折点),手绘转箭头要识别形态,AI 能生成弯曲/折线。弯曲箭头 base(commit a708eb1)此前已落地但缺形态切换 UI / 折线 / 识别 / AI 生成。本轮补全箭头形态系统。spec 内部 spec。
...
用户实测打包版反馈「画布完全不能用」三问题,逐个修。计划 内部 plan。
...
放大核心卖点「转义」(画布↔文字 DSL 双向,任何 AI 可驱动画布)。三个功能都是转义的直接产物,各自独立。计划 内部 plan,subagent 串行执行(Markdown→双向桥→diff,逐个 commit)。
...
收口审计/主线三件可实现遗留。计划 内部 plan,subagent 编排串行(三者都触 canvas page.tsx 不同段,按序 commit 避冲突)。
...
审计遗留的三类「静默吞 QuotaExceeded」同源问题,复刻 `db-client.ts` 已验证的配额契约模式(`saveXxx(): boolean` + `onQuotaExceeded` 订阅点)统一收口。计划 内部 plan,5 步 TDD(subagent 编排,每步一 commit)。
