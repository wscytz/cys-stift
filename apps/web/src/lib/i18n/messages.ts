...
export const messages = {
  ...
  'cmd.seeAll': { zh: '在搜索页查看全部 →', en: 'See all in Search →' },
  ...
  'card.detail.saved': { zh: '已保存', en: 'Saved' },
  ...
  'card.detail.deleteConfirmTitle': { zh: '删除这张卡片？', en: 'Delete this card?' },
  'card.detail.deleteConfirmBody': { zh: '卡片将移入回收站（从画布和列表隐藏），可在 /trash 恢复。', en: 'The card moves to Trash (hidden from canvas and lists) — recoverable from /trash.' },
  'card.detail.deleteConfirmAction': { zh: '移入回收站', en: 'Move to Trash' },
  'card.detail.discardTitle': { zh: '丢弃未保存的编辑？', en: 'Discard unsaved edits?' },
  'card.detail.discardBody': { zh: '你在这张卡上还有没保存的修改。现在关闭会丢失这些编辑。', en: 'You have unsaved changes on this card. Closing now will lose them.' },
  'card.detail.discardAction': { zh: '丢弃编辑', en: 'Discard' },
  ...
  'canvas.exportDoneDetail': { zh: '已导出 {name}（{format} · {scope}）', en: 'Exported {name} ({format} · {scope})' },
  ...
  'canvas.dslErrPosFormat': { zh: '指令含 @pos 但参数格式不对——应为 @pos(x, y)，如 @pos(80, 80)', en: '@pos is present but its arguments are malformed — use @pos(x, y), e.g. @pos(80, 80)' },
  'canvas.dslErrPosMissing': { zh: '卡片缺 @pos(位置)——每张卡都要有 @pos(x, y)', en: 'Card is missing @pos — every card needs @pos(x, y)' },
  'canvas.dslDiscardTitle': { zh: '丢弃未应用的编辑？', en: 'Discard unsaved edits?' },
  'canvas.dslDiscardBody': { zh: '你改动了 DSL 但还没应用。现在关闭会丢弃这些编辑。', en: 'You edited the DSL without applying. Closing now discards those edits.' },
  'canvas.dslDiscardAction': { zh: '丢弃', en: 'Discard' },
  ...
  'agent.contentChanges': { zh: '内容修改 {n}', en: 'Content changes {n}' },
  'agent.contentNewCard': { zh: '新卡', en: 'New card' },
  'agent.contentFieldTitle': { zh: '标题', en: 'Title' },
  'agent.contentFieldBody': { zh: '正文', en: 'Body' },
  ...
  'settings.capabilities.title': { zh: '能力清单', en: 'Capabilities' },
  'settings.capabilities.lede': { zh: '这个应用能做什么:核心能力默认开启,可选能力按需启用。', en: 'What this app can do — core is always on, optional is opt-in.' },
  'settings.capabilities.kindCore': { zh: '核心', en: 'Core' },
  'settings.capabilities.kindOptional': { zh: '可选', en: 'Optional' },
  'capabilities.status.core': { zh: '核心 · 默认开启', en: 'Core · always on' },
  'capabilities.status.ready': { zh: '就绪', en: 'Ready' },
  'capabilities.status.notReady': { zh: '未配置', en: 'Not configured' },
  'capabilities.status.on': { zh: '已开启', en: 'On' },
  'capabilities.status.off': { zh: '已关闭', en: 'Off' },
  'capabilities.hint.ai': { zh: '在 AI 设置中配置 →', en: 'Configure in AI settings →' },
  'capabilities.hint.labs': { zh: '在实验室中开启 →', en: 'Enable in Labs →' },
  'capabilities.canvas.label': { zh: '画布组织', en: 'Canvas organize' },
  'capabilities.canvas.desc': { zh: '空间排列、连接、绘制、多画布', en: 'Spatial layout, links, drawing, multiple canvases' },
  'capabilities.relations.label': { zh: '关系', en: 'Relations' },
  'capabilities.relations.desc': { zh: '关系箭头、图谱、反向链接', en: 'Relation arrows, graph, backlinks' },
  'capabilities.dsl.label': { zh: '转义 DSL', en: 'DSL' },
  'capabilities.dsl.desc': { zh: '画布↔文字双向、AI 可驱动', en: 'Canvas↔text, AI-driven' },
  'capabilities.compute.label': { zh: '@compute', en: '@compute' },
  'capabilities.compute.desc': { zh: '只读几何公式', en: 'Read-only geometry formulas' },
  'capabilities.structuredFields.label': { zh: '结构化字段', en: 'Structured fields' },
  'capabilities.structuredFields.desc': { zh: '@type / @tags / @links / @code / @quote', en: '@type / @tags / @links / @code / @quote' },
  'capabilities.search.label': { zh: '全文搜索', en: 'Search' },
  'capabilities.search.desc': { zh: '标题 / 正文 / 链接 / 代码', en: 'Title / body / links / code' },
  'capabilities.exportRestore.label': { zh: '导出与恢复', en: 'Export & restore' },
  'capabilities.exportRestore.desc': { zh: 'JSON 备份、导入前恢复点', en: 'JSON backup, pre-import restore point' },
  'capabilities.localFirst.label': { zh: '本地优先', en: 'Local-first' },
  'capabilities.localFirst.desc': { zh: '数据在本地、无遥测、AI 可选', en: 'Local data, no telemetry, AI optional' },
  'capabilities.ai.label': { zh: 'AI 增强', en: 'AI' },
  'capabilities.ai.desc': { zh: '摘要 / 改写 / 翻译 / 关系推荐 / 画布工作流', en: 'Summarize, rewrite, translate, relate, canvas workflows' },
  'capabilities.proposalCoauthor.desc': { zh: '可审计 AI 共编(实验室)', en: 'Auditable AI coauthor (Labs)' },
  ...
  'settings.importReading': { zh: '正在解析备份…', en: 'Reading backup…' },
  ...
  'settings.importErrorCheckpoint': { zh: '无法创建导入前恢复点（存储空间不足）。可跳过恢复点继续，但导入失败时将无法自动回滚。', en: 'Could not create the pre-import recovery point (storage full). You can skip it and continue, but a failed import won’t auto-rollback.' },
  ...
  'settings.importCheckpointAvailable': { zh: '导入前状态快照可恢复：{createdAt} · {cards} 张卡片 · {media} 个媒体 · {bytes}。后续导入失败不会覆盖这份快照。', en: 'A pre-import recovery snapshot is available: {createdAt} · {cards} cards · {media} media · {bytes}. A later failed import will not overwrite it.' },
  'settings.importCheckpointRestore': { zh: '恢复导入前状态', en: 'Restore pre-import state' },
  'settings.importCheckpointDelete': { zh: '删除恢复点', en: 'Delete recovery point' },
  'settings.importCheckpointDeleteTitle': { zh: '删除恢复点？', en: 'Delete recovery point?' },
  'settings.importCheckpointDeleteBody': { zh: '删除后无法恢复导入前状态（当前数据不变）。导入失败时的自动回滚仍会新建快照。', en: 'You will no longer be able to restore the pre-import state (current data stays). A failed import will still create a fresh snapshot.' },
  'settings.importCheckpointDeleted': { zh: '恢复点已删除。', en: 'Recovery point deleted.' },
  'settings.importCheckpointConfirmTitle': { zh: '恢复导入前状态？', en: 'Restore the pre-import state?' },
  'settings.importCheckpointConfirmBody': { zh: '这会用导入前保存的完整快照（{createdAt}）替换当前卡片、画布、媒体和设置。此操作不可撤销——恢复后导入以来新增的数据将永久丢失。快照成功恢复后清理；失败则保留。确定恢复？', en: 'This replaces current cards, canvases, media, and settings with the snapshot from before your import ({createdAt}). This cannot be undone — data added since the import is permanently lost. The snapshot clears after a successful restore; it stays if recovery fails. Continue?' },
  ...
  'search.noMatchWithFilter': { zh: '当前筛选下未找到匹配 "{q}" 的卡片。', en: 'No cards match "{q}" under the current filters.' },
  'search.clearFilter': { zh: '清除筛选', en: 'Clear filters' },
  ...
  'capture.dropImportJsonHint': { zh: '这是 cy\'s Stift 的全量备份。要恢复它，请到 设置 → 数据 → 导入 JSON 选择该文件。', en: "That's a cy's Stift full backup. To restore it, go to Settings → Data → Import JSON and pick this file." },
  ...
  'ai.errorAuth': { zh: 'AI 认证失败 — 检查该 profile 的 API key 与 baseUrl 是否正确。', en: 'AI authentication failed — check this profile’s API key and baseUrl.' },
  'ai.errorModel': { zh: 'AI 模型不可用 — 确认模型名拼写;若用 Ollama,先运行 ollama pull <模型>。', en: 'AI model unavailable — check the model name; for Ollama run `ollama pull <model>` first.' },
  'ai.errorRateLimit': { zh: 'AI 请求被限流或超出配额 — 稍后再试。', en: 'AI request was rate-limited or over quota — try again later.' },
