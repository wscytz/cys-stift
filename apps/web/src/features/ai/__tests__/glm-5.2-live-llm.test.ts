 ...
 * - key 从 process.env.DEEPSEEK_API_KEY 读,**只进 Authorization header**,
 *   绝不写进 prompt / 日志 / 断言 / 错误信息。(GLM 不启用:结果不可信。)
...
/** 选 provider:DeepSeek(v4-flash)。env 可覆盖 baseUrl/model。GLM 不启用(结果不可信)。 */
...
async function callModel(cfg: ProviderCfg): Promise<string> {
  ...
  const res = await fetch(`${cfg.baseUrl}/chat/completions`, {
    ...
    body: JSON.stringify({
      ...
      stream: false,
      max_tokens: 512,
      // DeepSeek v4 思考会吃光 token 致空输出(与 apps/web openai-provider 一致),关思考。
...
describe.skipIf(!RUN_LIVE)('glm-5.2 live LLM —— 真实模型输出可被吃下(默认跳过)', { timeout: 90000 }, () => {
  it('模型输出经 graceful parser 产出 ≥1 op,sanitize 不抛(不断言原文)', async () => {
    ...
    if (!cfg) {
      console.warn('live-llm: CYS_DSL_LIVE_LLM=1 但未配 DEEPSEEK_API_KEY,跳过')
      return
    }
    ...
    // 直接喂 graceful parser(模型输出常带围栏/散文,strict 对散文仍报;真实 AI 路径走 graceful)。
